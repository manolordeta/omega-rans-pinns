...
# =============================================================================
# Configuration -- small first run to verify everything works
# =============================================================================
CONFIG = {
    ...
    'y_range': (-3.0, 3.0),
    't_range': (0.0, 1.0),
    # Network architecture (small for fast compilation)
...
def make_loss_fn(A, nu, w_pde, w_ic):
    ...
    def loss_fn(model, batch):
        ...
        def ic_error_sq(x, y, t):
            xyt = jnp.stack([x, y, t])
            # omega_tilde = -laplacian(psi_tilde)
            # Use hessian for IC too (consistent with residual computation)
            def psi_scalar(xyt_):
...
def main():
    ...
    loss_history = []
    print("=" * 60)
    print("omega-RANS Forward Solve with PINN")
    print("=" * 60)
    print(f"  Architecture: {cfg['hidden_dims']}")
    print(f"  Stuart A={cfg['A']}, nu={cfg['nu']}")
    print(f"  Steps: {cfg['n_steps']}, LR: {cfg['lr']}")
    print(f"  Interior pts: {cfg['n_interior']}, IC pts: {cfg['n_initial']}")
    print()
    print("Compiling JIT (first step will be slow)...")
    ...
    print("Evaluating omega_tilde on grid...")
    ...
    fig.suptitle(r"$\tilde{\omega}(x, y, t)$ — omega-RANS forward", fontsize=14)
    fig.tight_layout()
    fig.savefig(os.path.join(output_dir, 'omega_tilde_evolution.png'),
                dpi=150, bbox_inches='tight')
    print("Saved omega_tilde_evolution.png")

    # Base flow vorticity for reference
    def base_omega(x, y, t):
        return vorticity(x, y, cfg['A'])

    base_field = evaluate_on_grid(base_omega, x_range, y_range, 0.0, nx=60, ny=60)
    ...
    print("Saved base_vorticity.png")
